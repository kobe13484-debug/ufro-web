...
export default function UFROCalculator() {
  ...
  const dilution = useMemo(() => {
    ...
    if (dilutionMode === 'auto') {
      ...
      if (Cd >= Ctarget) return { needed: true, rejectFails, autoMode: true, cannotSolve: true, Cd, Cr, Qr, Ctarget, sourceFlows: makeSourceFlows(0), msg: `Conductivity น้ำผสมเฉลี่ย (${Math.round(Cd)}) สูงกว่าเป้าหมาย (${Math.round(Ctarget)} µS/cm) ไม่สามารถลดค่าได้` };
      if (Cr <= Ctarget) return { needed: true, rejectFails, autoMode: true, cannotSolve: false, QdReq: 0, Cd, Cr, Qr, Ctarget, finalFlow: Qr, finalCond: Cr, finalTDS: cond2tds(Cr), finalStatus: getRejectStatus(cond2tds(Cr)), finalV: validateDischarge(cond2tds(Cr)), srcName, sourceFlows: makeSourceFlows(0) };
      ...
      return { needed: true, rejectFails, autoMode: true, cannotSolve: false, QdReq, Cd, Cr, Qr, Ctarget, finalFlow, finalCond, finalTDS, finalStatus: finalV.severityStatus, finalAllowed: finalV.regulatoryAllowed, finalV, srcName, sourceFlows: makeSourceFlows(QdReq) };
    } else {
