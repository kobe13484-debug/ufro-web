...
export default function UFROCalculator() {
  ...
  const dilution = useMemo(() => {
    ...
    if (dilutionMode === 'auto') {
      const active = dilutionSources.filter(s=>s.enabled);
      const Cd = active.length > 0
        ? active.reduce((sum, s) => sum + toNumber(s.conductivity), 0) / active.length
        : 500;
      const srcName = active.length === 0 ? 'น้ำผสม' : active.length === 1 ? active[0].name : `${active.length} แหล่งน้ำผสม`;
      const makeSourceFlows = (flow) => active.map(s => ({
        ...s,
        actualFlow: active.length > 0 ? flow / active.length : 0,
        actualRatio: active.length > 0 ? 100 / active.length : 0,
      }));
      if (Cd >= Ctarget) {
        const sourceFlows = makeSourceFlows(0);
        return { needed: true, rejectFails, autoMode: true, cannotSolve: true, Cd, Cr, Qr, Ctarget, sourceFlows, mixingSources: sourceFlows, msg: `Conductivity น้ำผสมเฉลี่ย (${Math.round(Cd)}) สูงกว่าเป้าหมาย (${Math.round(Ctarget)} µS/cm) ไม่สามารถลดค่าได้` };
      }
      if (Cr <= Ctarget) {
        const sourceFlows = makeSourceFlows(0);
        return { needed: true, rejectFails, autoMode: true, cannotSolve: false, QdReq: 0, Cd, Cr, Qr, Ctarget, finalFlow: Qr, finalCond: Cr, finalTDS: cond2tds(Cr), finalStatus: getRejectStatus(cond2tds(Cr)), finalV: validateDischarge(cond2tds(Cr)), srcName, sourceFlows, mixingSources: sourceFlows };
      }
      ...
      const sourceFlows = makeSourceFlows(QdReq);
      return { needed: true, rejectFails, autoMode: true, cannotSolve: false, QdReq, Cd, Cr, Qr, Ctarget, finalFlow, finalCond, finalTDS, finalStatus: finalV.severityStatus, finalAllowed: finalV.regulatoryAllowed, finalV, srcName, sourceFlows, mixingSources: sourceFlows };
    } else {
      ...
      return { needed: true, rejectFails, autoMode: false, cannotSolve: false, dilFlow, finalFlow, finalCond, finalTDS, finalStatus: finalV.severityStatus, finalAllowed: finalV.regulatoryAllowed, finalV, Cr, Qr, sources: active, mixingSources };
    }
                      ...
                      <div key={ds.id} style={{...S.srcCard,...(ds.enabled?S.srcCardOn:{}),marginBottom:4}}>
                        ...
                        {ds.enabled && (
                          <div style={{display:'grid',gridTemplateColumns:'1fr',gap:4,marginTop:6}}>
                            ...
                            <div style={S.srcField}><label style={S.srcFieldLabel}>Auto Flow</label>
                              <div style={{...S.srcInputWrap,...S.srcInputRO}}>
                                <NumInput value={parseFloat((vol(dilution.sourceFlows?.find(x=>x.id===ds.id)?.actualFlow || 0)).toFixed(1))} onValueChange={()=>{}} style={S.srcInput} readOnly/>
                                <span style={S.srcUnit}>{volUnit}</span>
                              </div>
                              {timeUnit==='hourly' && <div style={S.srcTdsHint}>≈ {fmt((dilution.sourceFlows?.find(x=>x.id===ds.id)?.actualFlow || 0)*opsHours,0)} m³/day</div>}
                            </div>
                          </div>
                        )}
                      </div>))}
            ...
            <div style={{overflowX:'auto'}}>
              <table style={S.table}><thead><tr>
                ...
                {dilution.needed && !dilution.cannotSolve && (dilution.finalFlow||0) > 0 && (<>
                  ...
                  {dilutionMode==='auto' && (dilution.QdReq||0) > 0 && dilution.sourceFlows?.map(ds =>
                    <StreamRow key={`dil-auto-${ds.id}`} name={`├─ Dilution: ${ds.name}`} flow={vol(ds.actualFlow)} tds={cond2tds(toNumber(ds.conductivity))} pct={ds.actualRatio || 0} sub/>)}
      ...
      <g opacity={dilZoneOpacity}>
        ...
        {dilSources.length > 0 ? (
          dilSources.map((ds, i) => {
            ...
            const laneGap = 24;
            const y = 458 - ((total - 1) * laneGap) / 2 + i * laneGap;
            const x0 = 150;
            const x1 = 445;
            const mid = 360;
            ...
            return (
              <g key={`dil-src-${ds.id || i}`}>
                <rect x={x0} y={y - 13} width="160" height="26" rx="3"
                  fill={rejectFails?'#0f2218':'#0d1814'}
                  stroke={rejectFails?'#5da377':'#2a4538'} strokeWidth={rejectFails?1:0.75}/>
                <text x={x0 + 8} y={y - 2} textAnchor="start" fill={rejectFails?'#9bc7a4':'#3a6049'} fontSize="8" fontFamily={mf} fontWeight="700">
                  {ds.name || `Source ${i+1}`}</text>
                <text x={x0 + 8} y={y + 10} textAnchor="start" fill={rejectFails?'#e8f0e8':'#2a4538'} fontSize="8" fontFamily={mf} fontWeight="600">
                  {`${fmt(vol(flow),1)} ${volUnit}${cond ? ` · ${Math.round(cond).toLocaleString()} µS/cm` : ''}`}</text>
                <path d={`M ${x0 + 160} ${y} L ${mid} ${y} L ${x1} 458`}
                  fill="none"
                  stroke={rejectFails?'#5da377':'#2a4538'}
                  strokeWidth={rejectFails?1.8:1}
                  markerEnd="url(#arr)"/>
              </g>
            );
          })
          ...
          <>
            ...
            <text x="365" y="444" textAnchor="end" fill={rejectFails?'#9bc7a4':'#3a6049'} fontSize="9" fontFamily={mf} fontWeight="600">น้ำผสม</text>
            <text x="365" y="458" textAnchor="end" fill={rejectFails?'#e8f0e8':'#2a4538'} fontSize="9" fontFamily={mf} fontWeight="600">
              {hasDilution ? `${fmt(vol(dilWaterFlow),1)} ${volUnit}` : 'Flow —'}</text>
            <text x="365" y="471" textAnchor="end" fill={rejectFails?'#7ba386':'#2a4538'} fontSize="8" fontFamily={mf}>Cond ต่ำ</text>
          </>
        )}

        {dilSources.length > 0 && (
          <>
            <text x="330" y="423" textAnchor="start" fill={rejectFails?'#d4a857':'#3a6049'} fontSize="8" fontFamily={mf} fontWeight="700">
              DILUTION WATER TOTAL</text>
            <text x="330" y="435" textAnchor="start" fill={rejectFails?'#f0d488':'#2a4538'} fontSize="9" fontFamily={mf} fontWeight="700">
              {`${fmt(vol(dilWaterFlow),1)} ${volUnit}`}</text>
          </>
        )}
